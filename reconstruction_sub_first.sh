...
for sub in $(ls ${rawDir}); do
	...
	1)
	    ...
	    cond_name=$(echo ${cond} | sed -e 's|/|_|g' -e 's|_$||')
	...
	0)
	    ...
	    cond=$(awk -F"," '($2=="'"${sub}"'") {for (i=3; i < NF; i++) print $i}' tmp_scanLog.csv | tr '\n' '/')
	    cond_name=$(echo ${cond} | sed -e 's|/|_|g' -e 's|_$||')
	    echo "running ${sub} which corresponds to sub${name}_${cond_name}"
	    echo "Processed: NO" >> preprocessing.log
	    echo "Processing sub${name}_${cond_name}" >> preprocessing.log 
	    ;;
	?)
    ...
    for scan in ${scans}; do
		...
		if [ -e "${preProcDataDir}/sub${name}/${cond}/${scan}/sub${name}_${cond_name}_${scan}.nii.gz" ]; then
		    echo "${scan} exists for sub${name}_${cond_name}, do you want to overwrite? yes/no"
		    read -t 10 ans
		    if [ "${ans}" == "yes" ]; then
			echo "rewriting scan ${scan}..."
		    elif [ "${ans}" == "no" ]; then
			echo "skipping scan ${scan}"
			echo "SKIPPING ${scan} for sub${name}_${cond_name}" >> preprocessing.log
			continue
		    else
			echo "skipping scan ${scan}"
			echo "SKIPPING ${scan} for sub${name}_${cond_name}" >> preprocessing.log
		...
		for sub_scan_folder in ${sub_scan_folders}; do
			...
			echo "Error: the number of DICOMs in ${sub_scan_folder} directory for sub${name}_${cond_name} do not match the number specified by ${scanVol}"
		...
		echo "CONVERTING DICOMS FROM ${correct_dir} FOR sub${name}_${cond_name}" >> preprocessing.log
	...
	####################################################################
		...
		if [ "${scan_strip}" == "DTI" ]; then
		    ...
		    if [ ! -e ${preProcDataDir}/sub${name}/${cond}/${scan}/*.bval ] || [ ! -e ${preProcDataDir}/sub${name}/${cond}/${scan}/*.bvec ]; then
		    	...
		    	mv ${preProcDataDir}/sub${name}/${cond}/${scan}/*.nii.gz ${preProcDataDir}/sub${name}/${cond}/${scan}/sub${name}_${cond_name}_${scan}.nii.gz
		    else
			    mv ${preProcDataDir}/sub${name}/${cond}/${scan}/*.bval ${preProcDataDir}/sub${name}/${cond}/${scan}/sub${name}_${cond_name}_${scan}.bval
			    mv ${preProcDataDir}/sub${name}/${cond}/${scan}/*.bvec ${preProcDataDir}/sub${name}/${cond}/${scan}/sub${name}_${cond_name}_${scan}.bvec
			    mv ${preProcDataDir}/sub${name}/${cond}/${scan}/*.nii.gz ${preProcDataDir}/sub${name}/${cond}/${scan}/sub${name}_${cond_name}_${scan}.nii.gz
			fi
	...
	####################################################################
		...
		#DOES NOT WORK WITH DTI DATA TO GET BVECS & BVALS
		    ...
		    mri_convert -it siemens_dicom -ot nii ${DICOM_FILE_EX} ${preProcDataDir}/sub${name}/${cond}/${scan}/sub${name}_${cond_name}_${scan}.nii.gz >> preprocessing.log	
		fi

		     #change orientation of image to what we use
		     #ORIENTATION CODE HERE
		     ############################################################
		     infile=${preProcDataDir}/sub${name}/${cond}/${scan}/sub${name}_${cond_name}_${scan}.nii.gz
	...
	####################################################################
		...
		if [[ ${tr} -gt 1500 ]] && [[ ${tr} -lt 3000 ]] && [[ ${te} -lt 50 ]] && [[ "${scan_type}" == "epfid2d1_64" ]]; then
		    ...
		    halfPoint=`fslhd ${preProcDataDir}/sub${name}/${cond}/${scan}/sub${name}_${cond_name}_${scan}_RPI.nii.gz | grep "^dim4" | awk '{print int($2/2)}'`
		    
			#Run 3dvolreg, save matrices and parameters
	      #Saving "raw" AFNI output for possible use later (motionscrubbing?)
		    3dvolreg -verbose -tshift 0 -Fourier -zpad 4 -prefix ${preProcDataDir}/sub${name}/${cond}/${scan}/motion/mcImg.nii.gz -base $halfPoint -dfile ${preProcDataDir}/sub${name}/${cond}/${scan}/motion/mcImg_raw.par -1Dmatrix_save ${preProcDataDir}/sub${name}/${cond}/${scan}/motion/mcImg.mat ${preProcDataDir}/sub${name}/${cond}/${scan}/sub${name}_${cond_name}_${scan}_RPI.nii.gz
	 ...
	 ##Need to create the absolute and relative displacement RMS measurement files
	    ...
	    #Create images of the motion correction (translation, rotations, displacement), mm and radians
	      #switched from "MCFLIRT estimated...." title
		    ...
		    mv ${preProcDataDir}/sub${name}/${cond}/${scan}/motion/disp.png ${preProcDataDir}/sub${name}/${cond}/${scan}/motion/${scan}_${cond_name}_${name}_disp.png 
		    cp ${preProcDataDir}/sub${name}/${cond}/${scan}/motion/${scan}_${cond_name}_${name}_disp.png ${preProcDataDir}/Func_Motion_Check/
		fi
	...
	####################################################################
		...
		echo "FINISHED ${scan} for sub${name}_${cond_name}" >> preprocessing.log
...
#update Log
    ...
    echo "FINISHED sub${name}_${cond_name}" >> preprocessing.log
